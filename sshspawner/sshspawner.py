...
class SSHSpawner(Spawner):
    ...
    users_config_path = Unicode(None,
            help=dedent("""Path to the users yaml config file where we can 
    ...
    def choose_remote_host(self):
        ...
        """
        self.log.debug("Selecting server for user.")
        if self.users_config_path:
            self.log.debug(f"users config path is set to {self.users_config_path}")
            with open(self.users_config_path, 'r') as stream:
                try:
                    config = yaml.safe_load(stream)
                    self.log.debug(config['users'][self.user.name])
                    return config['users'][self.user.name]
                except Exception as e:
                    self.log.error(e)
        self.log.debug("no user config file found or error, default to random")
